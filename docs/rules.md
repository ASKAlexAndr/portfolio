...
## Icons
- По умолчанию — `lucide-react` (предпочтительно). Размеры и цвет — через Tailwind классы.
...
## Дополнительные правила оптимизации

### Статичные массивы вне компонента
- **Выносить статичные массивы на уровень модуля**
    - Если создается массив в `useMemo` и он не зависит от пропсов/состояния/контекста — вынесите массив на уровень модуля.
  ```typescript
  // ❌ Плохо: статичный массив в useMemo
  function Component() {
    const icons = useMemo(() => [<IconA/>, <IconB/>], []); // вынести за компонент
    return <div>{icons}</div>;
  }

  // ✅ Хорошо: модульная константа
...
### Избегать useCallback для простых обработчиков
- **Функции без зависимостей выносить на уровень модуля**
    - Функции, не зависящие от пропсов/состояния, лучше объявлять на уровне модуля, а не в `useCallback([])`.
  ```typescript
  // ❌ Плохо: useCallback для простой функции
  function Component() {
    const handle = useCallback(() => doSomething(), []); // лучше вынести в модуль
    return <button onClick={handle}>Click</button>;
  }

  // ✅ Хорошо: модульная функция
...
- **Избегать избыточных комментариев**
    - Не писать очевидные/избыточные комментарии, которые повторяют код
    - Комментируйте «почему», а не «что»
    - Улучшайте читаемость кода вместо добавления комментариев
  ```typescript
  // ❌ Плохо: очевидный комментарий
  const count = count + 1; // increments count

  // ✅ Хорошо: объяснение причины
  // Using +1 to avoid off-by-one when moving to the next page in pagination
